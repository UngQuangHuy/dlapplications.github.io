...
Ở [phần 1](https://dlapplications.github.io/2018-06-11-perceptron/) của ~~series~~ (TODO: series naò?), chúng ta đã tìm hiểu về perceptron - mô hình đơn giản nhất của một artificial neuron (tế bào thần kinh nhân tạo). ~~Trong phần tiếp theo này, nhóm sẽ giới thiệu tới các bạn mô hình đơn giản nhất của một Artifial Neural Network (ANN: mạng thần kinh nhân tạo) được xây dựng nên từ các perceptron này~~ (TODO: ANN và perceptron có liên quan gì ? có thể đặt thêm một câu hỏi : ANN là cha tập hợp các perceptron, vậy tại sao lại cần tập hợp lại). Mô hình này có tên là Multi-layer Perceptron (MLP) và ~~thường được biểu diễn như hình dưới~~ (TODO: đây là một ví dụ ).<br/>
![ANN](/img/20180611/ANN.jpg) 
> Hình 1. Multi-layer Perceptron

Gọi là Multi-layer Perceptron (perceptron nhiều lớp) bởi vì nó là tập hợp của các perceptron ~~được sắp xếp thành nhiều hơn 2 lớp~~ (TODO: chai làm nhiều nhóm, mỗi nhóm tương ứng với một layer). Trong hình trên ta có một ANN với 3 lớp: Input layer (lớp đầu vào), Output layer (lớp đầu ra) và Hidden layer (lớp ẩn). ~~Vì khi giải quyết một bài toán người dùng cuối thường chỉ quan tâm đến input và output của một mô hình~~ (TODO: tất cả mọi người cũng chỉ quan tâm tới input và output thôi. A nghĩ do User chỉ chạm tới được lớp input, output) , do vậy trong ANN ngoài lớp Input và Output ra thì các lớp neuron ở giữa được gọi chung là Hidden (ẩn: không phải là không nhìn thấy mà đơn giản là không quan tâm đến). 
...
Để dễ hình dung, người ta thường gọi lớp Input là lớp thứ nhất (kí hiệu $l=1$) cứ thế tăng dần cho tới lớp output (trong Hình 1 là $l=3$). Ta sẽ nói ANN này có số lớp là $L=3$. Mỗi hình tròn là biểu trưng cho một ~~neuron~~ (TODO: trên là perceptron, dưới là neuron rồi ....) do đó đối với lớp thứ 2 ta nói nó có $n=5$ neuro, lớp thứ 3 có $n=2$ neuron. Đối với lớp input thì mỗi hình tròn chỉ là đại diện cho một giá trị input chứ không phải là một neuron. Để biểu diễn một ANN bằng công thức toán, người ta thường sử dụng các kí hiệu như sau:
...
TODO: đoan này a nghĩ nên viết lại.Theo kiểu lấy một perceptron trong mỗi layer ra, sau đấy viết công thức cho nó.  

Sử dụng các kí hiệu như trên chúng ta sẽ biểu diễn quá trình tính toán từ input cho ra output của một ~~MLP 3 lớp~~ (TODO: trên là ANN dưới lại MLP rồi)  trong Hình 1. Với lớp $l=1$ thì chỉ là lớp của các giá trị input nên ta không tính toán gì ở đây. Với lớp $l=2$ ta sẽ tính output của từng neuron. Bắt đầu với neuron thứ 1 của lớp $l=2$, như đã tìm hiểu ở [phần 1](https://dlapplications.github.io/2018-06-11-perceptron/) đầu tiên chúng ta sẽ tính tổng các input tới đã được điều chỉnh độ mạnh yếu nhờ các giá trị $w$ (gọi là $z$, cách kí hiệu giống như với $a$):
...
Và ta có thể làm tương tự như vậy đối với các neuron tiếp theo trong lớp Hidden $l=2$ và lớp Output $l=3$. Ta có thể thấy các tính toán trên giống hệt nhau cho các neuron trong cùng một lớp ~~và không có gì quá phức tạp~~ (TODO: câu này thừa). Vì lý do đó mà ta có thể tính toán song song (parallel) nhiều neuron trong một lớp để tăng tốc độ. Điều đó dẫn tới việc trong DL ~~thì sử dụng GPU có tốc độ tính toán nhanh hơn nhiều so với CPU~~ (GPU hay được sử dụng để tăng tốc độ tính toàn (training + testing). ~~Lý do là GPU có nhiều core hơn rất nhiều so với CPU nên phù hợp cho tính toán song song. Mặc dù mỗi core của GPU có tốc độ chậm hơn rất nhiều so với mỗi core của CPU nhưng với các tính toán đơn giản trên thì vẫn dư sức để chạy~~(TODO: một CPU thông thường có khoảng vài chục core với xung nhịp (frequency) mỗi core lớn. Một GPU thông thường sẽ có khoảng vài trăm tới vài nghìn core với xung nhịp mỗi core nhỏ hơn.   ).<br/>
...
TODO: ở phía trên activation function là cho mỗi neurol, ở đây lại lấy nó làm activation function cho lớp ???? . 
...
TODO: đoạn này đọc rất khó hiểu. Em dùng một cái table để biểu thị bài toán ra được không
...
TODO: a nghĩ cả phần 4 cần tách ra làm một bài riêng. A đọc đoạn này cũng không hiểu em viết gì. 
